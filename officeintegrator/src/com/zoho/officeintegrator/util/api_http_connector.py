...
class APIHTTPConnector(object):
    ...
    def fire_request(self, converter_instance):
        ...
        if self.content_type is not None and not isinstance(converter_instance, FormDataConverter):
        ...
        if self.request_method == Constants.REQUEST_METHOD_GET:
            ...
            if self.file:
                response = requests.post(url=self.url, files=data, params=self.parameters, headers=self.headers,
