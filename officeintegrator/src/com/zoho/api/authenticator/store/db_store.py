...
class DBStore(TokenStore):
    ...
    def save_token(self, token):
        cursor = None
        if not isinstance(token, OAuth2):
            return
        try:
            connection = mysql.connector.connect(host=self.__host, database=self.__database_name, user=self.__user_name,password=self.__password, port=self.__port_number)
            oauthToken = token
            ...
            try:
                ...
                if cursor.rowcount <= 0:
                    ...
                    query = "insert into " + self.__table_name + " (id,user_name,client_id,client_secret,refresh_token,access_token,grant_token,expiry_time,redirect_url) values (%s,%s,%s,%s,%s,%s,%s,%s,%s);"
                    val = (token.get_id(), token.get_user_signature().get_name() if token.get_user_signature() is not None else None, token.get_client_id(), token.get_client_secret(), token.get_refresh_token(), token.get_access_token(), token.get_grant_token(), token.get_expires_in(), token.get_redirect_url())
        ...
        try:
            ...
            try:
                ...
                for result in results:
                    oauth_token = object.__new__(OAuth2)
    ...
    def find_token_by_id(self, id):
        ...
        try:
            ...
            try:
                ...
                oauthToken = object.__new__(OAuth2)
